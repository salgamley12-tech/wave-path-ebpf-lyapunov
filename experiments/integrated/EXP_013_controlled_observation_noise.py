"""EXP-013: Controlled observation-noise integration.

Extends EXP-012 by introducing deterministic Gaussian observation noise.
...
OUTPUT_CSV = "experiments/integrated/EXP_013_controlled_observation_noise.csv"
OUTPUT_JSON = "experiments/integrated/EXP_013_controlled_observation_noise.json"
...
def main():
    ...
    # 1. Generate the deterministic wave model.
    wave_rows = run(cfg)

    # 2. Add controlled deterministic observation noise.
    signal = [(row[0], row[1]) for row in wave_rows]
    observations = observe(signal, sigma=sigma, seed=seed)
    ...
    # 3. Build Wave Path using observed values.
    path = WavePath()
    rows = []
    ...
    result = {
        "experiment": "EXP-013",
        "description": (
            "Controlled observation-noise integration using the actual "
            "wave model, observation layer, Lyapunov calculation, "
            "and Wave Path trajectory."
        ),
        "configuration": {
            "seconds": cfg.seconds,
            "dt": cfg.dt,
        ...
        "validation": {
            ...
            "noise_present": max_abs_noise > 0.0,
            "rmse_positive": observation_rmse > 0.0,
